...
from backend.api.dependencies import CurrentUser, graph_queries
from backend.api.presenters import clamp_limit
from backend.api.schemas import GraphDocumentResponse, GraphOverviewResponse
...
def document(user: CurrentUser, source: str, limit_chunks: int = 18) -> GraphDocumentResponse:
    owner_id = None if is_admin(user.role) else user.id
    payload = graph_queries.get_document_graph(source, clamp_limit(limit_chunks), owner_id)

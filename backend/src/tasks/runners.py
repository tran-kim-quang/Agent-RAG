...
class ChatTaskRunner:
    def __init__(self, chats: ChatRepository, chat_service: ChatService) -> None:
        self.chats = chats
    ...
    def run(self, run_id: str, chat_session_id: str, user_id: str, message: str, attempt: int, worker_id: str) -> None:
        ...
        with agent_run_monitor.bind_run(run_id), agent_run_monitor.bind_event_sink(event_sink), bind_user(user_id):
            ...
            answer = self.chat_service.answer(message, chat_session_id, history)
...
class UploadTaskRunner:
    ...
    def run(self, job_id: str, user_id: str, file_name: str, object_key: str, attempt: int, worker_id: str) -> None:
        now = datetime.now(timezone.utc)
        self.uploads.update(job_id, status="processing", phase="download", message=f"Worker is downloading '{file_name}' from object storage.", attempt_count=attempt, worker_id=worker_id, heartbeat_at=now, started_at=now, progress=2)

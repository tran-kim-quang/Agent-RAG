...
class GraphQueryService:
    def __init__(
        self,
        list_documents_fn: Callable[[int, str | None], list[dict]] = list_graph_documents,
        get_document_graph_fn: Callable[[str, int, str | None], dict] = get_document_graph,
    ) -> None:
    ...
    def list_documents(self, limit: int = 20, owner_id: str | None = None) -> list[dict]:
        return self._list_documents_fn(limit, owner_id)

    def get_document_graph(self, source: str, limit_chunks: int = 18, owner_id: str | None = None) -> dict:
        return self._get_document_graph_fn(source, limit_chunks, owner_id)
